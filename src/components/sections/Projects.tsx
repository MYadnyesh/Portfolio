...
const linkClass =
  "inline-flex items-center gap-2 font-mono text-xs tracking-meta uppercase text-fg-muted hover:text-accent-readable transition-colors focus-visible:outline-none focus-visible:outline-2 focus-visible:outline-gold";
...
function IndexRow({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="group border-t border-border transition-[padding,background-color] duration-300 hover:bg-bg-card px-0 hover:pl-4 md:hover:pl-8 py-10 md:py-12"
    >
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
            <h3 className="font-display font-bold text-3xl md:text-4xl lg:text-5xl leading-none tracking-[-0.02em] text-fg group-hover:text-accent-readable transition-colors">
              {project.name}
            </h3>
            {roleWord && (
              <span className="text-serif-italic text-2xl md:text-3xl text-accent-readable">
                {roleWord}
              </span>
            )}
            <ArrowUpRight
              className="w-6 h-6 text-fg-subtle transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-accent-readable"
              aria-hidden="true"
            />
          </div>

          <p className="text-lg md:text-xl font-medium text-fg-muted leading-relaxed text-pretty max-w-2xl">
            {project.tagline}
          </p>
        ...
        <div className="lg:col-span-4 flex flex-col gap-4 lg:items-end lg:text-right">
          <div className="font-mono text-xs uppercase tracking-meta text-fg-subtle flex gap-4 lg:justify-end">
            <span className="text-fg">{project.status}</span>
...
export function Projects() {
  return (
    <section id="projects" className="section bg-bg-elevated">
      <div className="max-w-360 mx-auto">
        ...
        >
          The{" "}
          <span className="text-serif-italic normal-case text-accent-readable">real</span>{" "}
          index
        </motion.h2>
        ...
        <motion.p
          ...
          className="mt-8 max-w-2xl text-lg text-fg-muted leading-relaxed text-pretty"
        >
          Every entry names its own role and tech honestly, no invented metrics, no borrowed logos.
          Repositories verified under{" "}
          <code className="font-mono text-fg">github.com/MYadnyesh</code>.
        </motion.p>
              ...
              {otherProjects.map((project, index) => (
                ...
                >
                  <div className="flex items-baseline gap-4 min-w-0">
                    ...
                    <span className="font-display text-lg md:text-xl truncate group-hover:text-accent-readable transition-colors">
                      {project.name}
                    </span>
                    <span className="font-mono text-xs text-fg-subtle hidden sm:inline">
                      {project.year}
                    </span>
                  </div>
