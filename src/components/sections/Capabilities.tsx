      ...
      <div className="container">
        ...
        >
          ...
          <h2 className="section-title mt-3 font-bold uppercase">
            The full <span className="text-serif-italic lowercase text-accent-readable">toolkit</span>
          </h2>
      ...
      <div className="flex flex-col gap-3 md:gap-4">
        {rows.map((row, i) => {
          ...
          return (
            <div
              key={row.key}
              className="group relative flex items-center border-y border-border py-4 md:py-6"
            >
              {/* Category label pinned left, over a fade */}
              <div className="absolute left-0 top-0 bottom-0 z-10 flex items-center pl-4 md:pl-8 pr-8 bg-linear-to-r from-bg via-bg to-transparent">
                <span className="index-num text-sm mr-3">{String(i + 1).padStart(2, "0")}</span>
                <span className="font-mono text-xs tracking-label uppercase text-fg-muted whitespace-nowrap">
                  {row.label}
                </span>
              </div>
