        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px bg-border mb-16">
          {aiPipeline.map((stage, index) => (
            ...
            >
              ...
              <p className="text-fg-subtle text-xs font-mono mt-1 mb-4">{stage.description}</p>
              <ul className="space-y-1.5" role="list">
                {stage.items.length > 0 ? (
                  stage.items.map((item) => (
                    <li key={item} className="text-base md:text-lg text-fg-muted">
                      {item}
                    </li>
                  ))
                ) : (
                  <li className="text-sm text-fg-subtle italic">&middot;</li>
                )}
              </ul>
            </motion.div>
          ))}
            ...
            {aiPrinciples.map((principle, index) => (
              ...
              >
                <Card padding="md" className="h-full">
                  ...
                  <p className="text-fg-muted text-base md:text-lg leading-relaxed">{principle.description}</p>
                </Card>
              </motion.div>
            ))}
          </div>
